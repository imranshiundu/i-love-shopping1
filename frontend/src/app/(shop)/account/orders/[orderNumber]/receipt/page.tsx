...
import { FiArrowLeft, FiPrinter, FiLoader, FiAlertCircle, FiCheckCircle } from 'react-icons/fi';

function ReceiptContent({ orderNumber }: { orderNumber: string }) {
  ...
  if (!order && !error) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-24 text-center print:hidden">
        ...
        <p className="mt-4 text-stone-500">Loading receipt...</p>
      </div>
    );
  }
  ...
  const payments: any[] = (order.payments || []).filter((p: any) => p.status === 'SUCCEEDED');
  const paidTotal = payments.reduce((sum: number, p: any) => sum + Number(p.amount || 0), 0);
  const fullyPaid = paidTotal >= Number(order.total || 0) && paidTotal > 0;
      ...
      <div className="rounded-2xl border border-stone-200 bg-white p-8 print:rounded-none print:border-0 print:p-0 sm:p-10">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b-2 border-stone-900 pb-6">
          ...
          <div className="text-right">
            <p className="text-3xl font-extrabold tracking-tight text-stone-900">RECEIPT</p>
            <p className="mt-1 font-mono text-sm font-bold text-stone-600">{order.number}</p>
            <p className="text-sm text-stone-500">Paid {formatDate(order.updatedAt || order.createdAt)}</p>
          </div>
        </div>

        <div className="mt-6 flex items-center gap-3">
          {fullyPaid ? (
            <span className="inline-flex items-center gap-2 rounded-full bg-emerald-100 px-4 py-1.5 font-extrabold uppercase tracking-wide text-emerald-700 print:border print:border-emerald-600">
              <FiCheckCircle /> Paid in full
            </span>
          ) : (
            <span className="inline-flex items-center gap-2 rounded-full bg-amber-100 px-4 py-1.5 font-extrabold uppercase tracking-wide text-amber-700 print:border print:border-amber-600">
              <FiAlertCircle /> {order.status}
            </span>
          )}
        </div>

        <div className="mt-6">
          <p className="text-xs font-bold uppercase tracking-wide text-stone-400">Received from</p>
        ...
        <dl className="ml-auto mt-6 w-full max-w-xs space-y-1.5 text-sm">
          ...
          <div className="flex justify-between border-t border-stone-900 pt-2 text-base font-extrabold">
            <dt>Total received</dt><dd className="tabular-nums">{formatKES(order.total)}</dd>
          </div>
        </dl>

        <div className="mt-8 rounded-xl bg-stone-50 p-4 ring-1 ring-stone-200 print:border print:border-stone-300">
          <p className="text-xs font-bold uppercase tracking-wide text-stone-400">Payment details</p>
          {payments.length === 0 && <p className="mt-2 text-sm text-stone-500">No completed payments recorded yet.</p>}
          <ul className="mt-2 space-y-2">
            {payments.map((p: any) => (
              <li key={p.id} className="flex flex-wrap items-center justify-between gap-2 text-sm">
                <span className="font-semibold">{p.provider} · {p.providerId}</span>
                <span className="tabular-nums">{formatKES(p.amount)} · {formatDate(p.createdAt)}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <p className="mt-8 border-t border-stone-200 pt-4 text-center text-xs text-stone-400">
          ...
          This is a computer-generated receipt. Thank you for shopping with us.
        </p>
      </div>
    </div>
  );
